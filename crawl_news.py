...
class news_crawler:
    def news_crawler(self, company, query_list, start_date, end_date=None):
        if end_date is None:
            end_date = start_date
            ...
            for query in query_list:
                ...
                while page < max_page:
                    ...
                    for i in range(temp_df.shape[0]):  # 본문 기사 크롤링
                        temp_df["기사 내용"].iloc[i, :] = self.get_contents(temp_df.iloc[i, :]["기사 링크"],
                                                                   temp_df.iloc[i, :]["언론사"],
    ...
    def get_contents(self, url, press, request_headers):
        try:
...
if __name__ == "__main__":
    ...
    arg_list = ['현대차', '기아 자동차', '20210101', '20210104']
    ...
    query_list = ["정의선", "아반떼",
                  ...
                  "현대차", "기아차", "코나EV", "아이오닉5"][:2]

    cl = news_crawler()
    df = cl.news_crawler(company, query_list, start_date, end_date)
    print(df)
